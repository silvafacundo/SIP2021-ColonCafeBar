...
module.exports = class ProductPUT extends Route {
	...
	async run(req, res, user) {
		const { productId, idCategory, name, description, price, isActive } = req.body;
		// Check if body parameters are valid
		try {
			// var product = await this.utils.products.getProduct(id);
			//if product doesn't exists, display a error message
			//update product
			await this.utils.products.updateProduct({ productId, idCategory, name, description, price, isActive });

...
module.exports = class ProductPOST extends Route {
	...
	async run(req, res, user) {
		const { idCategory, name, description, imageUrl, price, variants } = req.body;
		...
		try {
			...
			await this.utils.products.createProduct({ idCategory, name, imageUrl, description, price, variants });
